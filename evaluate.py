...
from bayes import NaiveBayesClassifier, visualize_words, DEFAULT_WINDOW_SIZE, DEFAULT_HAM_THRESHOLD, visualize_data_summary, visualize_top_words
...
def main(_data: DataFrame,
    ...
    model.evaluate_classification(classification, ham_threshold=_ham_threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--chunk-words", type=int,
                        help="N-gram/number of words to chunk together, defaults to " + str(DEFAULT_CHUNK_WORDS) + ".",
                        default=DEFAULT_CHUNK_WORDS,
                        required=False)
    parser.add_argument("-w", "--window-size", type=int,
                        help="The size of the rolling window used for classification, defaults to " + str(DEFAULT_WINDOW_SIZE) + ".",
                        default=DEFAULT_WINDOW_SIZE,
                        required=False)
    parser.add_argument("-ht", "--ham-threshold", type=float,
                        help="The threshold to be exceeded for classifying something as spam, defaults to " + str(DEFAULT_HAM_THRESHOLD) + ".",
                        default=DEFAULT_HAM_THRESHOLD,
                        required=False)

    parser.add_argument("--stopwords", type=bool,
                        help="Preprocess with stopwords and lemmatization, defaults to True.",
                        default=True,
                        required=False)
    parser.add_argument("--substitution", type=bool,
    ...
    main(data, args.model_path, args.chunk_words, preprocessors, args.window_size, args.ham_threshold)
